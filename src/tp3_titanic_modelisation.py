...
INPUT_PATH = Path("data/processed/tp1_titanic_model_ready.csv")
...
def evaluate_binary_model(
    model_name: str, y_true: pd.Series, y_pred: pd.Series, y_prob: pd.Series | None
) -> dict:
    """Compute standard classification metrics for the final report."""
    result = {
        "model": model_name,
...
def main() -> None:
    ...
    model_metrics = evaluate_binary_model(
        "random_forest", y_test, model_pred, model_prob
    )

    metrics_df = pd.DataFrame([baseline_metrics, model_metrics])
